...
/*--------------------------------------*/
	...
	// Set segment name passed as String into specified place mesgStruktur-Array.
	private void setMesgSegment(String nachricht, int reihenfolge) {
		...
		// Prüft ob Stelle an der Segment eingefügt wird größer als die maximal Anzahl in Struktur ist oder an erster Stelle stehen soll.
		if (reihenfolge > currsize) { 
			this.priv_meth_newsegnumber("So viele Segmente gibt es in der Struktur nicht. Bitte geben Sie eine Zahl kleiner " + currsize + " ein.");
	...
	// true => Neuer Eintrag wurde gesetzt. false => Kein neuer Name wurde vergeben, nicht möglich.
	@SuppressWarnings("unused")
...
/*--------------------------------------*/
	...
	private int getsegLaenge(String seg) {
		...
		switch (seg) {
			case "hea": segLaenge = healaenge;
						break;
			case "pat":	segLaenge = patlaenge;
						break;
			case "fal": segLaenge = fallaenge;
						break;
			case "ana": segLaenge = analaenge;
						break;
			case "mfr": segLaenge = mfrlaenge;
			break;
			case "mle":	segLaenge = mlelaenge;
						break;
			case "egt":	segLaenge = egtlaenge;
						break;
			case "rsf":	segLaenge = rsflaenge;
						break;
			case "rsb":	segLaenge = rsblaenge;
						break;
			case "end": segLaenge = endlaenge;
						break;
			default:	segLaenge = 0;
						break;
		}
